...
trait ChannelStateTestsBase extends Assertions with Eventually {
  ...
  def reachNormal(setup: SetupFixture, tags: Set[String] = Set.empty, interceptChannelUpdates: Boolean = true): Transaction = {
    ...
    val initiatorRequestRemoteFunding_opt = if (liquidityAds) {
      ...
      Some(LiquidityAds.RequestRemoteFunding(TestConstants.nonInitiatorFundingSatoshis, maxFee, BlockHeight(TestConstants.defaultBlockHeight), LiquidityAds.DEFAULT_LEASE_DURATION))
    } else {
      None
    }
